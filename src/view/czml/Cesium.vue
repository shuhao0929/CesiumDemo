...
onMounted(async () => {
  ...
  viewer._cesiumWidget._creditContainer.style.display = 'none' // 隐藏版权底部logo
  ...
  handler.setInputAction(function (movement) {
    ...
    if (Cesium.defined(position)) {
      let cartographicPosition = Cesium.Ellipsoid.WGS84.cartesianToCartographic(position)
      ...
      let cartesianPosition = viewer.scene.globe.pick(ray, viewer.scene)
      console.log('经度（Longitude）: ' + longitude)
      console.log('纬度（Latitude）: ' + latitude)
      console.log('笛卡尔坐标（Cartesian）: ' + cartesianPosition)
    } else {
      console.log('未能获取有效的位置信息')
    }
